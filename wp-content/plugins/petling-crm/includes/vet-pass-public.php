...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['vet_action']) ) {
    ...
    // Ενημέρωση Βάρους από Γιατρό (Χωρίς πεδίο γιατρού)
    if ( $_POST['vet_action'] === 'log_weight' ) {
        $new_weight = sanitize_text_field($_POST['weight']);
        
        $wpdb->insert( $wpdb->prefix . 'petling_vet_notes', array( 'pet_unique_id' => $pet_id, 'weight' => $new_weight, 'vet_comment' => 'Τακτική Ζύγιση', 'status' => 'verified', 'created_by' => 'vet' ), array( '%s', '%f', '%s', '%s', '%s' ) );
    ...
    if ( $_POST['vet_action'] === 'verify_vaccine' ) {
        ...
        $wpdb->update( $wpdb->prefix . 'petling_vaccines', array('status' => 'verified', 'vaccine_name' => sanitize_text_field($_POST['vaccine_name']), 'date_administered' => sanitize_text_field($_POST['date_administered']), 'next_vaccine_date' => sanitize_text_field($_POST['next_vaccine_date']), 'vet_name' => sanitize_text_field($_POST['vet_name'])), array('id' => $vac_id) );
        wp_redirect( $_SERVER['REQUEST_URI'] ); exit;
    }
    ...
    if ( $_POST['vet_action'] === 'add_vaccine' ) {
        $wpdb->insert( $wpdb->prefix . 'petling_vaccines', array( 'pet_unique_id' => $pet_id, 'vaccine_name' => sanitize_text_field($_POST['vaccine_name']), 'date_administered' => sanitize_text_field($_POST['date_administered']), 'next_vaccine_date' => sanitize_text_field($_POST['next_vaccine_date']), 'vet_name' => sanitize_text_field($_POST['vet_name']), 'status' => 'verified', 'created_by' => 'vet' ), array( '%s', '%s', '%s', '%s', '%s', '%s', '%s' ) );
        wp_redirect( $_SERVER['REQUEST_URI'] ); exit;
    }
...
if ( ! $current_pet ) wp_die( 'Το κατοikίδιο δεν βρέθηκε.', 'Petling Vet Pass' );
...
<head>
    ...
    <style>
        ...
        .weight-timeline { display:flex; gap:15px; overflow-x:auto; padding-bottom:10px; margin-top:15px; }
        .weight-card { background:#f9f9f9; border:1px solid #e5e5e5; padding:10px 15px; border-radius:8px; min-width:90px; text-align:center; }
    </style>
</head>
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            foreach ($vet_notes as $n) {
                if (!empty($n->weight)) { $weight_history[] = array('date' => $n->created_at, 'weight' => $n->weight); }
            }
            ...
            ?>
                ...
                <?php foreach($weight_history as $wh): ?>
                    <div class="weight-card">
                        <div style="font-size:12px; color:#666; margin-bottom:4px;"><?php echo date('d/m/Y', strtotime($wh['date'])); ?></div>
                        <div style="font-weight:bold; color:#43282F; font-size:16px;"><?php echo esc_html($wh['weight']); ?> <span style="font-size:12px;">kg</span></div>
                    </div>
                <?php endforeach; ?>
                </div>
            <?php endif; ?>
                ...
                <div class="item-box" style="<?php if($vac->status==='verified') echo 'border-left:4px solid #5b9a68;'; else echo 'border-left:4px solid #e6a23c;'; ?>">
                    <?php if ( $vac->status === 'verified' ) : ?>
                        ...
                        <div style="font-size: 13px; color: #666; margin-top: 5px;">
                            ...
                            <?php if ( !empty($vac->next_vaccine_date) ) echo '<span style="color:#d63638; font-weight:bold;">Επόμενο: ' . date('d/m/Y', strtotime($vac->next_vaccine_date)) . '</span>'; ?>
                        </div>
                    <?php else : ?>
                        <div style="color:#e6a23c; font-weight:bold; margin-bottom:10px;">⏳ Προσχέδιο (Επεξεργασία & Κλείδωμα):</div>
                        <form method="POST" action="">
                            ...
                            <input type="date" name="date_administered" value="<?php echo esc_attr($vac->date_administered); ?>" style="width:100%; box-sizing:border-box; padding:8px; border:1px solid #ccc; border-radius:4px; margin-bottom:10px;">
            ...
            <?php if ( $vet_notes ) : foreach ( $vet_notes as $note ) : ?>
                <div class="item-box" style="<?php if($note->status==='verified') echo 'border-left:4px solid #5b9a68;'; else echo 'border-left:4px solid #e6a23c;'; ?>">
                    <?php if ( $note->status === 'verified' ) : ?>
                        ...
                        <?php if ( !empty($note->next_exam_date) ) echo '<div style="margin-top: 8px; color:#d63638; font-size: 13px; font-weight:bold;">Επανεξέταση: ' . date('d/m/Y', strtotime($note->next_exam_date)) . '</div>'; ?>
                    <?php else : ?>
                        <div style="color:#e6a23c; font-weight:bold; margin-bottom:10px;">⏳ Προσχέδιο (Επεξεργασία & Κλείδωμα):</div>
                        <form method="POST" action="">
